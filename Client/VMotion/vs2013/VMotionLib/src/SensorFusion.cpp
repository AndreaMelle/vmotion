...
void SensorFusion::matrixUpdate(const Eigen::Vector3f& accel, const Eigen::Vector3f& gyro)
{
	...
	updateMatrix(0,0) = 0;
	updateMatrix(0,1) = -G_Dt*Omega_Vector(2);//-z
	updateMatrix(0,2) = G_Dt*Omega_Vector(1);//y
	updateMatrix(1,0) = G_Dt*Omega_Vector(2);//z
	updateMatrix(1,1) = 0;
	updateMatrix(1,2) = -G_Dt*Omega_Vector(0);//-x
	updateMatrix(2,0) = -G_Dt*Omega_Vector(1);//-y
	updateMatrix(2,1) = G_Dt*Omega_Vector(0);//x
	updateMatrix(2,2) = 0;
...
void SensorFusion::driftCorrection()
{
	...
	Accel_magnitude = sqrtf(Accel_Vector(0) * Accel_Vector(0) + Accel_Vector(1) * Accel_Vector(1) + Accel_Vector(2) * Accel_Vector(2));
	Accel_magnitude = Accel_magnitude / GRAVITY; // Scale to gravity.
	// Dynamic weighting of accelerometer info (reliability filter)
	// Weight for accelerometer info (<0.5G = 0.0, 1G = 1.0 , >1.5G = 0.0)
	Accel_weight = constrain(1 - 2.0f * fabsf(1.0f - Accel_magnitude), 0, 1.0f);  //  
	...
	mag_heading_x = cosf(magHeading);
	mag_heading_y = sinf(magHeading);
...
void SensorFusion::eluerAngles()
{
	...
	roll = atan2f(dcmMatrix(2,1), dcmMatrix(2,2));
	yaw = atan2f(dcmMatrix(1,0), dcmMatrix(0,0));
}
